...
#include <pcap.h>
#include <arpa/inet.h>
#include "firewall.h"  // 包含防火墙函数的头文件
...
void pcapCallback(u_char *user, const struct pcap_pkthdr *header, const u_char *pkt_data) {
    ...
    for (const auto &pattern : patterns) {
        if (matchPattern(pattern, onepacket.packetcontent, algorithm)) {
            ...
            inet_ntop(AF_INET, onepacket.src_ip, src_ip_str, INET_ADDRSTRLEN);

            // 阻断源 IP 地址
            refuse(src_ip_str);
            std::cout << "Intrusion detected from IP " << src_ip_str << ". IP has been blocked." << std::endl;
        }
    }
